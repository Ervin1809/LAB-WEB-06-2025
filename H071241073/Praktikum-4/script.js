  ...
  if (keadaan.balance <= 0) {
    ...
    overlayBox.innerHTML = `
      <div style="text-align:center;">
        <h2 style="margin-bottom:6px;">💀 GAME OVER 💀</h2>
        <p>Saldo habis!</p>
        <button id="restartBtn" class="btn-blue" 
          style="padding:8px 16px;margin-top:10px;border:none;border-radius:8px;cursor:pointer;">
          Mulai Ulang
        </button>
      </div>
    `;
    ...
    setTimeout(() => {
      ...
      if (restartBtn) restartBtn.addEventListener('click', mulaiUlangGame);
    }, 100);
    return; 
  }
...
function mulaiUlangGame() {
  keadaan.balance = 5000;
  keadaan.lastRoundWinner = null;
  keadaan.roundOver = false;
  overlayFull.style.display = 'none';
  tampilNotifikasi('Game dimulai ulang! Saldo kembali ke $5000', 2500, '#27ae60');
  perbaruiTampilan();
}
