    ...
    <div class="rounded-xl border border-slate-700 bg-slate-800/60 backdrop-blur-sm shadow-xl shadow-cyan-500/10 overflow-hidden">
        <div class="border-b border-slate-700 bg-gradient-to-r from-slate-800/50 to-slate-900/50 p-6 relative">
            ...
            <div class="relative flex items-center space-x-3">
                <i class="bi bi-pencil-square text-2xl text-yellow-400"></i>
                <h2 class="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-400 font-orbitron tracking-wide">EDIT FISH: {{ strtoupper($fish->name) }}</h2>
            </div>
        </div>
        <form action="{{ route('fishes.update', $fish) }}" method="POST" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            <div class="space-y-2">
                <label for="name" class="block text-sm font-medium text-cyan-400 font-orbitron tracking-wide">Fish Name</label>
                <input type="text" name="name" id="name" value="{{ old('name', $fish->name) }}" class="mt-1 block w-full rounded-lg border border-slate-600 bg-slate-900/70 text-slate-200 shadow-sm focus:border-cyan-500 focus:ring-cyan-500 py-3 px-4 transition-all duration-300" placeholder="Enter fish name" required>
                @error('name')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" step="0.01" name="base_weight_max" id="base_weight_max" value="{{ old('base_weight_max', $fish->base_weight_max) }}" class="mt-1 block w-full rounded-lg border border-slate-600 bg-slate-900/70 text-slate-200 shadow-sm focus:border-cyan-500 focus:ring-cyan-500 py-3 px-4 transition-all duration-300" placeholder="0.00" required>
            ...
            <div class="space-y-2">
                ...
                <input type="number" name="sell_price_per_kg" id="sell_price_per_kg" value="{{ old('sell_price_per_kg', $fish->sell_price_per_kg) }}" class="mt-1 block w-full rounded-lg border border-slate-600 bg-slate-900/70 text-slate-200 shadow-sm focus:border-cyan-500 focus:ring-cyan-500 py-3 px-4 transition-all duration-300" placeholder="0" required>
            ...
            <div class="space-y-2">
                ...
                <input type="number" step="0.01" name="catch_probability" id="catch_probability" value="{{ old('catch_probability', $fish->catch_probability) }}" class="mt-1 block w-full rounded-lg border border-slate-600 bg-slate-900/70 text-slate-200 shadow-sm focus:border-cyan-500 focus:ring-cyan-500 py-3 px-4 transition-all duration-300" placeholder="0.00" required>
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-lg border border-slate-600 bg-slate-900/70 text-slate-200 shadow-sm focus:border-cyan-500 focus:ring-cyan-500 py-3 px-4 transition-all duration-300" placeholder="Enter fish description">{{ old('description', $fish->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-3 pt-5 border-t border-slate-700/50">
                <button type="submit" class="w-full sm:w-auto px-6 py-3 rounded-lg bg-gradient-to-r from-yellow-600 to-orange-600 text-black hover:from-yellow-700 hover:to-orange-700 transition-all duration-300 font-orbitron shadow-lg shadow-yellow-500/20">
                    <i class="bi bi-arrow-repeat mr-2"></i> Update Fish
                </button>
